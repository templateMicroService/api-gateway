...
public class Routes {
    ...
    public RouterFunction<ServerResponse> orderServiceRoutes() {
        return GatewayRouterFunctions.route("order_service")
                ...
                .filter(CircuitBreakerFilterFunctions.circuitBreaker("orderServiceBreaker", URI.create("forward:/fallbackRoute")))
                .build();
    }
    ...
    public RouterFunction<ServerResponse> fallbackRoute() {
        return GatewayRouterFunctions.route("fallbackRoute")
                .GET("/fallbackRoute", request -> ServerResponse.status(
                        HttpStatus.SERVICE_UNAVAILABLE).body("Service Unavailable, please try again later")
                )
                .build();
    }
}
